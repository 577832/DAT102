...
public class TabellMengde<T> implements MengdeADT<T> {
	...
	public MengdeADT<T> union(MengdeADT<T> m2) {
		...
		for (int i = 0; i < antall; i++) {
			((TabellMengde<T>) begge).settInn(tab[i]); // Går igjennom this-mengden og legger alle elementene til i den
														// nye mengden begge,
		...
		while (teller.hasNext()) {
			T element = teller.next(); // Går igjennom m2 - mengden så lenge det er elementer i mengden.
			if (!begge.inneholder(element)) {// settet inn elementet i mengden begge så lenge det er en plass på neste
												// plass..
	...
	public MengdeADT<T> snitt(MengdeADT<T> m2) {
		...
		while (teller.hasNext()) {
			element = teller.next(); // Hvis elementet finnes i this mengden og i m2 mengden så settes elementet inn
