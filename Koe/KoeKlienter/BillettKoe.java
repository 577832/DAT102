...
public class BillettKoe{	
	...
	public static void main(String[]a){
		Kunde kunde;
		TabellKoe<Kunde> kundekoe = new TabellKoe<Kunde>();
		...
		// Simulerer alle alternativ fra 1 til MAKS_ANT_LUKER billett-luker
			...
			while(!kundekoe.erTom()){
				for (int lukenr = 0; lukenr < ant_luker; lukenr++){
					if(!kundekoe.erTom()){
						...
						if(kunde.hentAnkTid()>lukeTid[lukenr]){// kunden har ventet
							start = kunde.hentAnkTid(); 
						}else{                                // kunden har ikke ventet 
							start = lukeTid[lukenr];    
						}
						ferdig = start + PROSESSTID;           //  legger til betjeningstid
						kunde.nyFerdigTid(ferdig);             //  kunden er betjent 
